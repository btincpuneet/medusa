...
## Magento Orders Proxy

- Route: `GET /admin/magento-orders`
- Purpose: fetches Magento B2C orders and exposes them to the Medusa admin UI (or any consuming client).
- Environment variables:
  - `MAGENTO_REST_BASE_URL` — the Magento REST root (include `/rest/V1`).
  - `MAGENTO_ADMIN_TOKEN` — a Magento admin/integration token with permission to read orders.
- Query parameters:
  - `page` (default `1`), `page_size` (default `20`).
  - Optional filters: `status`, `customer_email`, `increment_id`.
- Response shape:
  - `page`, `pageSize`, `totalCount`, `orders` (Magento order payload), and `raw` (full Magento response for advanced use).

Configure the two environment variables, then call the endpoint from the Medusa admin (for example, wire the Orders tab to `GET /admin/magento-orders`).
